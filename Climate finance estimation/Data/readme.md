...
| (not in the paper) fine-grained 18-class report | `classification_reportmulticlassifier.csv` | `Training and Classifying/multi-classifier.py` | Yes |
| Climate topics used to build the training sample (appendix table built from BERTopic output) | `topic_info.csv` | **Producing code not included** (see below) | Yes |
...
**Classifier reports.** Each report CSV has the layout of scikit-learn's `classification_report(..., output_dict=True)` written as a table. The header is `,precision,recall,f1-score,support`. There is one row per class, followed by `accuracy`, `macro avg` and `weighted avg`. The scripts write these files to this folder when they run. The three shipped files are the reports from the training run used in the paper. With them, the performance table can be rebuilt without retraining the transformers, which requires a GPU. A new run changes the train/validation/test split and fine-tuning, so its numbers can differ slightly from these files.

**`topic_info.csv` (BERTopic topic table).** This is the topic-information table from the unsupervised BERTopic clustering of OECD CRS project descriptions (columns `Topic, Count, Name, CustomName, Representation, Zephyr, Representative_Docs`). The clustering is the upstream step that also produced `projects_clusters.csv`, and it comes from a separate study cited in the paper. **The code that produced `topic_info.csv` is not included in this package.** The file is shipped as a documented intermediate artifact. The manuscript keeps only the climate topics it lists and truncates long text before building the appendix table.
